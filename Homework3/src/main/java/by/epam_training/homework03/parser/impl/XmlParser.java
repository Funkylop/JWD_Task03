...
public class XmlParser implements FileParser {
    ...
    @Override
    public List<String> filterData(List<String> stringsXml){
    ...
    @Override
    public Element buildFileStructure(List<String> stringsOfFilteredXmlFile) {
        ...
        for (String stringXmlFile : stringsOfFilteredXmlFile){
            ...
            nameEndTag = searchClosingTag(stringXmlFile);
            ...
            }else if (nameEndTag.isEmpty()){
                stackOfNodes.add(node);
            }

            String nameLastNode = stackOfNodes.get(stackOfNodes.size()-1).getName();
            if (!nameEndTag.isEmpty()){
                if ((nameLastNode.charAt(0) + ADDING_SLASH_TO_NAME_TAG + nameLastNode.substring(1)).equals(nameEndTag)
                        || nameLastNode.equals(nameEndTag)){
